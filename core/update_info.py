...
class GroupInfo:
    ...
    async def get(self) -> dict:
        ...
        group_info = await bot.get_group_info(group_id=self.group_id)
        personal_info = await bot.get_group_member_info(group_id=self.group_id, user_id=QID)
...
async def update_all_group_info():
    ...
    for id in all_group:
        ...
        if json.load(open(GROUP_INFO_PATH/f"{id}.json", 'r', encoding='utf-8')).keys() != group_info.default().keys():
